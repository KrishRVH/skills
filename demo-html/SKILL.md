...
before/after screenshots of the real running application, with every piece of
copy written twice — plain language and technical — behind a page-level
...
3. **Capture "before".** Stash only the changed files
   (`git stash push -- <paths>`), clear any compiled-view/template caches,
   reshoot the same targets, then `git stash pop` and clear caches again. If
   you added any temporary access shim (login route, auth bypass) for capture,
   remove it and prove it is gone (request it, expect 404) before finishing.
   Done when before-images match the after-shots in framing, the working tree
   is back to its pre-capture state, and no shim responds.
